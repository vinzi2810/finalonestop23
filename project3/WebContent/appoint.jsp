...
$(document).ready(function(){
   ...
   // $('#hospital').change(function(){
	  $('body').on('input','.SearchBy',function(){
	...
	var v = "";
	 ...
	 if ($("#SearchBy").val() != '') {
           $.ajax({
               url: 'MyData/' + 'hospitalNameSelection',
              ...
              // data: {'name' : "done"  },
               ...
               success: function (result) {
                  ...
                  for (i = 0; i < result.length; i++) { 
                	...
                	//  alert (result[i]);
                	  //r += '<p>' + result[i] + '</p><br>' ;
                	  hospitalList =  hospitalList+ "<tr><td id='name'>" + result[i].name + "</td><td>" + result[i].address1+ "</td><td>" + result[i].city + "</td></tr>";
                	  //alert (hospitalList);
                	
                	  //var markup = "<tr><td><input type='checkbox' name='record'></td><td>" + name + "</td><td>" + subject + "</td></tr>";
                	}
                  hospitalList = "<table id='tablehospital'><thead><tr><th>Hospital</th><th>Locality</th><th>City</th></thead><tbody>"+hospitalList;
...
$('body').on("click", "#tablehospital tbody tr ", function(e) {

    //var rowdata = $(this).text();
    
    e.preventDefault();
    var hosname =$(this).find('td:eq(0)').text();
    var locality = $(this).find('td:eq(1)').text();
    var city = $(this).find('td:eq(2)').text();
    //var rowdata = hosname + ","+ locality + "," + city;
    var rowdata = hosname;
    //location.reload(true);
    //alert (city);
    $("#SearchBy").val(rowdata);
   // alert(rowdata);
    $("#tablehospital").remove();
   // alert ($(this).find('td:eq(0)').text());
    ...
    $.ajax({
        ...
        async: true,
        data: "hosname="+hosname+"&locality="+locality +"&city="+city,
        ...
        success: function(result) {
             ...
             for (i = 0; i < result.length; i++) { 
           	...
           	//  alert (result[i]);
           	  ...
           	  doctorList =  doctorList+ "<tr><td id='name'>" + result[i].doctorname + "</td><td>" +  result[i].gender + "</td><td>" + result[i].qualification + "</td><td>" + result[i].speciality +"</td><td style='display : none;'>"+ result[i].doctorid +  "</td></tr>" ;
           	
           	  //var markup = "<tr><td><input type='checkbox' name='record'></td><td>" + name + "</td><td>" + subject + "</td></tr>";
           	}
             doctorList  = "<table id='doctortable'><thead><tr><th>Doctor</th><th>Gender</th><th>Qualification</th><th>Speciality</th></thead><tbody>"+doctorList;
...
/*$('#SearchDoctors').on('input', function() {
	...
	*/
	...
	$('body').on("click", "#doctortable tbody tr ", function() {
	    //some think
	    //var rowdata = $(this).text();
	    var rowdata =  $(this).find('td:eq(0)').text()+ "," + $(this).find('td:eq(3)').text() ;
	    var doctorid = $(this).find('td:eq(4)').text();
	    var doctorname = $(this).find('td:eq(0)').text();
	...
	$('#slot').on('click', function() {
		...
		//alert( $('#SearchDoctors').val());

		
		 if (($("#SearchBy").val() != '')&&($('#SearchDoctors').val() != '')&&($('#mydate').val() != '')) {
	               ...
	               success: function(result) {
	                   ...
	                   for (i = 0; i < result.length; i++) { 
	                 	...
	                 	//  alert (result[i]);
	                 	  ...
	                 	  doctorList =  doctorList+"<option value=" +result[i]+ ">" + result[i] +"</option>";
	                 	
	                 	  //var markup = "<tr><td><input type='checkbox' name='record'></td><td>" + name + "</td><td>" + subject + "</td></tr>";
	                 	}
	                   doctorList = "<select name='slots'>"+doctorList;
	       		       doctorList = doctorList+ "</select>";
	                   //alert (doctorList);
	                   $("#slotselecting1").html(doctorList);
...
<form id="appointment_form" action="appointment/bookAppointment" method="POST">

<label for="SearchBy">Hospital</label>
<input class="SearchBy" type = "text" id ="SearchBy"  name = "hospital"/>

<br>
<div id="dvHospitals"></div>
<br>


<label for="SearchDoctors">Doctor</label>
<input type = "text" id ="SearchDoctors"   name="doctor"/>

<br>
<div id="doctorSelection"> </div>
 

<br>

<label for="mydate">Date</label>
<input type="text" id="mydate" gldp-id="mydate" path="date_selected" name="date" />
...
<div id = "slotselecting1">
<input type = "text" id ="slot" name="slots" />
</div>
